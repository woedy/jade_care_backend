...
class DoctorNotificationConsumers(AsyncJsonWebsocketConsumer):
    ...
    async def connect(self):
        ...
        self.data = {}
        self.user_data = None
        self.doctor_notifications = None


        await self.accept()

    async def receive_json(self, content):
        print("DoctorNotificationConsumers: receive_json")
        ...
        try:
            if command == "get_notifications":
                ...
                self.room_group_name = 'get_notifications_%s' % user_id
                ...
                await self.channel_layer.group_send(
                    self.room_group_name,
                    {
                        ...
                        'get_all_notifications_message': self.doctor_notifications
                    }
                )
    ...
    async def get_all_notifications(self, user_id, data):
        ...
        try:
            ...
            self.doctor_notifications = json.loads(notifications)
        except ClientError as e:
            await self.handle_client_error(e)
...
def get_all_notifications_or_error(user_id, data):
    ...
    try:
        notifications = Notification.objects.filter(doctor=user).order_by('-timestamp')
        serializers = ListDocNotificationSerializer(notifications, many=True)
...
def set_read_notification_or_error(user_id, notification_id):
    ...
    try:
        ...
        if notification.doc_read == False:
            notification.doc_read = True
            notification.save()
